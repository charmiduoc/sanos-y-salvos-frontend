...
class ImageService {
  ...
  async getMetadata(imageId: string): Promise<ImageResponse> {
    const response = await authFetch(`${this.baseUrl}/${imageId}`);
    if (!response.ok) throw new Error('Imagen no encontrada');
    return response.json();
  }

  async download(imageId: string): Promise<Blob> {
    const response = await authFetch(`${this.baseUrl}/download/${imageId}`);
  ...
  async exists(imageId: string): Promise<boolean> {
    const response = await authFetch(`${this.baseUrl}/${imageId}/exists`);
    return response.json();
  }

  async delete(imageId: string): Promise<void> {
    await authFetch(`${this.baseUrl}/${imageId}`, {
      method: 'DELETE'
    });
  }
}
